...
La pérdida total tiene tres componentes ([`total`](../physics/pde_loss.py#L177-L191)):

$$\mathcal{L} = w_{\mathrm{res}}\,\mathcal{L}_{\mathrm{res}} + w_{\mathrm{bc}}\,\mathcal{L}_{\mathrm{bc}} + w_{\mathrm{ic}}\,\mathcal{L}_{\mathrm{ic}}$$

| Componente | Qué penaliza | Detalle |
|---|---|---|
| $\mathcal{L}_{\mathrm{res}}$ | Incumplir la EDP en el interior | media de $r_u^2 + r_v^2$ sobre puntos de colocación interiores |
| $\mathcal{L}_{\mathrm{bc}}$ | Incumplir las condiciones de contorno | empotramiento ($u=v=0$) + superficies libres ($\sigma_{yy}=\tau_{xy}=0$) + extremo cargado ($\sigma_{xx}=0$, $\tau_{xy}=\tau_{\mathrm{app}}$) |
| $\mathcal{L}_{\mathrm{ic}}$ | Incumplir el reposo inicial | $u=v=0$ **y** $u_t=v_t=0$ en $t=0$ |

- Todas son **errores cuadráticos medios (MSE)** de cantidades que deberían ser
  cero. La solución exacta daría $\mathcal{L}=0$.
- **Pesos $w$:** $w_{\mathrm{res}}=1$, $w_{\mathrm{bc}}=10$, $w_{\mathrm{ic}}=10$
  ([config.py](../config.py#L175-L178)). Las condiciones de contorno/iniciales se
  ponderan ×10 porque, si no, la red encuentra el "mínimo tramposo" $u=v=0$ en
  todas partes (que cumple el residuo pero ignora la carga). Reforzar CC/CI la
  obliga a respetar el empotramiento, el reposo y la carga aplicada. Equilibrar
...
Cantidades ([config.py](../config.py#L164-L167)): 4000 interiores, 1000 por
frontera, 1500 iniciales. Son los puntos donde se exige que se cumpla la física.
...
$$U = \frac{u}{U_{\mathrm{ref}}}, \quad V = \frac{v}{U_{\mathrm{ref}}}, \qquad T_{\mathrm{ref}} = L_{\mathrm{ref}}\sqrt{\frac{\rho}{E}}, \quad \sigma_{\mathrm{scale}} = \frac{3\,P_{\mathrm{ref}}}{4\,c_{\mathrm{ref}}}, \quad U_{\mathrm{ref}} = \frac{L_{\mathrm{ref}}\,\sigma_{\mathrm{scale}}}{E}$$
...
**Entrada** ($d_{\mathrm{in}} = 7$):

$$\underbrace{(\xi,\; \eta,\; \hat{t})}_{\text{coordenadas}} ,\quad \underbrace{(\hat L,\; \hat c,\; \hat p,\; \hat t_{\mathrm{imp}})}_{\text{parámetros geométricos y de carga}}$$

**Salida** ($d_{\mathrm{out}} = 2$): $(U, V)$ — desplazamientos adimensionales.

- Las **3 coordenadas** $(\xi, \eta, \hat{t})$: dónde y cuándo. Respecto a ellas se
  derivan los residuos.
- Los **4 parámetros** $(\hat L, \hat c, \hat p, \hat t_{\mathrm{imp}})$: longitud, semi-altura,
  amplitud de carga e instante de impacto. **Esto es lo que hace la red PARAMÉTRICA** (§6).
- Las 7 entradas se **normalizan a $\approx[-1,1]$** con buffers $(lo, hi)$
...
con forma $(N,7) \to (N,\,k{=}5,\,7)$. Así cada punto se convierte en una
...
## 6. Sobre qué generaliza la red (y por qué es su gran ventaja)

Una PINN clásica resuelve **un** problema concreto: una geometría, una carga.
Cambiar $L$ o $P_{\max}$ obligaría a **reentrenar** desde cero. Aquí se da el salto
a una **PINN paramétrica**:
...
- [ ] Las tres componentes de la pérdida y por qué $w_{\mathrm{bc}}=w_{\mathrm{ic}}=10$ (evitar la solución trivial).
