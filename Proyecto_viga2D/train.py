...
def main():
    ...
    def compute_grads():
        ...
        for p in model.parameters():
            p.grad = None
        comps = {"res": 0.0, "bc": 0.0, "ic": 0.0}
        _accumulate("res", loss_fn.loss_residual, ent.w_res, "res", comps)
        for b, f in (("clamp", loss_fn.loss_clamp), ("load", loss_fn.loss_load),
        ...
        total = ent.w_res * comps["res"] + ent.w_bc * comps["bc"] + ent.w_ic * comps["ic"]
        return total, comps

    def _save_if_best(val):
        if val < best["loss"]:
            best.update(loss=val,
            ...
            for ep in range(args.adam):
                ...
                if ep % args.log_every == 0 or ep == args.adam - 1:
                    ...
                    print(f"  [A] ep {ep:5d}/{args.adam} | loss={total:.3e} "
                          f"| res={comps['res']:.2e} bc={comps['bc']:.2e} ic={comps['ic']:.2e} "
    ...
    payload = {
        ...
        "T_hat": dom.T_hat,
        "alpha_ramp": dom.alpha_ramp,
    }
    torch.save(payload, ent.checkpoint)
    print(f"\n[ok] mejor pérdida = {best['loss']:.3e}")
    print(f"[ok] modelo guardado en {ent.checkpoint}")

    # Subida opcional a W&B (gratuita) para usar el modelo desde la app local
    # sin mantener encendido el Pod de RunPod. Se activa con WANDB_API_KEY.
    try:
        from utils.registry import upload_checkpoint
        upload_checkpoint(ent.checkpoint, metadata={
            "best_loss": best["loss"],
        ...
        print(f"[wandb] subida omitida: {exc}")
