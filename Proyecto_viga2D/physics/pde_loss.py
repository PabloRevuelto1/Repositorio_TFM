...
class ElastodynamicsLoss:
    ...
    def total(self, sets):
        ...
        l_bc = (self.loss_clamp(sets["clamp"])
                + self.loss_load(sets["load"])
                + self.loss_free_surface(sets["top"])
                + self.loss_free_surface(sets["bottom"]))
        l_ic = self.loss_initial(sets["ic"])

        loss = self.w_res * l_res + self.w_bc * l_bc + self.w_ic * l_ic
        return loss, {"res": l_res.item(), "bc": l_bc.item(), "ic": l_ic.item()}
