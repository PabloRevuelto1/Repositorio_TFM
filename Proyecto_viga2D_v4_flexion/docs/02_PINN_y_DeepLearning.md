...
- **Pesos $w$:** $w_{\mathrm{res}}=1$, $w_{\mathrm{load}}=20$, $w_{\mathrm{bc}}=5$,
  $w_{\mathrm{ic}}=10$, $w_{\mathrm{eq}}=3$ ([config.py](../config.py)). La **carga** aleja
  del "mínimo tramposo" $u=v=0$; el **equilibrio** lo remata (con relajación temporal);
  $w_{\mathrm{ic}}$ es alto porque ahora impone la **velocidad inicial** $u̇(0)=0$, motor de
  la oscilación (ver doc 04 §12.4). Equilibrar estos pesos es uno de los puntos finos —y
  dificultades conocidas— de las PINN.
