...
def main():
    ...
    model = PINNsformer(
        ...
        in_lo=lo, in_hi=hi,
        hard_bc=mod.hard_constraints, ic_tau=mod.ic_tau, hard_ic=mod.hard_ic,
    ).to(device)
    ...
    try:
        # ===================== FASE A: Adam (CURRÍCULO TEMPORAL) =====================
        # El Adam se entrena en ETAPAS con ventana [0, t_frac·T_hat] creciente: la red
        # clava primero el transitorio inicial y luego lo extiende, respetando la
        # causalidad temporal (como el FEM, que marcha en el tiempo) para que emerja la
        # oscilación. El "mejor" modelo se decide sólo en la VENTANA COMPLETA (última
        # etapa), porque las etapas cortas tienen pérdidas más bajas y no comparables.
        if args.adam > 0:
            stages = ent.curriculum_stages
            print(f"\n[fase A] Adam · {args.adam} épocas · lr={ent.adam_lr} · chunk={chunk} "
                  f"· currículo de {len(stages)} etapas")
            opt = Adam(model.parameters(), lr=ent.adam_lr)
            t0 = time.time()
            ep_global = 0
            for si, (t_frac, ep_frac) in enumerate(stages):
                last = (si == len(stages) - 1)
                t_max = dom.T_hat if last else t_frac * dom.T_hat
                n_ep = (args.adam - ep_global) if last else int(round(ep_frac * args.adam))
                # Resamplear la ventana de esta etapa (la CI vive siempre en t̂=0).
                sets = to_tensor(build_training_sets(
                    dom, ent, k=mod.k, step=mod.seq_step,
                    t_max=(None if last else t_max)), device)
                if last:
                    best["loss"] = float("inf")   # el mejor se decide en la ventana completa
                print(f"  [A·currículo {si+1}/{len(stages)}] t̂∈[0,{t_max:.0f}] · {n_ep} épocas",
                      flush=True)
                for ep in range(n_ep):
                    total, comps = compute_grads()
                    opt.step()
                    history.append({"step": ep_global, "loss": total, **comps})
                    if last:
                        _save_if_best(total, comps)
                    if ep_global % args.log_every == 0 or ep == n_ep - 1:
                        el = time.time() - t0
                        its = (ep_global + 1) / el
                        print(f"  [A] ep {ep_global:5d}/{args.adam} | loss={total:.3e} "
                              f"| res={comps['res']:.2e} load={comps['load']:.2e} "
                              f"bc={comps['bc']:.2e} ic={comps['ic']:.2e} eq={comps['eq']:.2e} "
                              f"| best={best['loss']:.2e} | {its:.1f} it/s", flush=True)
                    ep_global += 1
                    if _should_stop():
                        break
                if _should_stop():
