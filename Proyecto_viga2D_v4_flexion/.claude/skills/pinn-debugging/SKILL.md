...
### 2-ter. Converge a la solución CUASI-ESTÁTICA (no captura la oscilación dinámica)
**Síntomas**: la flecha media es correcta (≈ estática/Euler-Bernoulli) pero la punta NO
rebota (baja y se queda); la "frecuencia" medida = ancho de ventana (artefacto); L2 vs FEM
se estanca ~50 %. Más épocas o pesos adaptativos no lo arreglan.
**Causa**: la oscilación es la **vibración libre** (modo propio) excitada al reconciliar la
carga con el reposo inicial. **Un modo libre tiene residuo de EDP = 0** → la pérdida es
INDIFERENTE a incluirlo. La solución cuasi-estática satisface residuo≈0, BCs, IC y equilibrio
seccional → es un mínimo legítimo y "simple", y ahí cae el optimizador. No es desbalance de
pesos (por eso los pesos adaptativos NO lo curan). El FEM sí oscila porque MARCHA en el
tiempo (causalidad incorporada).
**Fix — inyectar CAUSALIDAD temporal**:
1. **CI de velocidad BLANDA**: el ansatz que anula `u̇(0)` por construcción (`t̂²/(t̂²+τ²)`)
   permite a la red fingir reposo sin oscilar. Cambiar a `t̂/(t̂+τ)` (anula sólo el
   desplazamiento) e imponer `u̇(0)=0` como pérdida (peso alto) → fuerza la reconciliación
   global = excita el modo libre.
2. **Currículo temporal**: entrenar Adam por etapas con ventana `[0, t_frac·T]` creciente →
   la red construye el transitorio en orden causal (como el FEM).
3. **Soltar el constraint cuasi-estático tardío**: si hay equilibrio seccional con objetivo
   cuasi-estático, relajarlo en t tardío (`exp(-(t-t_ramp)/τ)`) para no suprimir la oscilación.
4. Si no basta: **pesos causales** (Wang et al., `w(t)=exp(-ε·Σ_{t'<t} residuo)`).
