...
class ModeloCfg:
    ...
    # τ de subida de la envolvente de la CI (en unidades t̂). Ver models/pinnsformer.py.
    ic_tau: float = 20.0
    # CI de VELOCIDAD: dura (envolvente t̂²/(t̂²+τ²) → u̇=0 por construcción) o BLANDA.
    # hard_ic=False usa la envolvente t̂/(t̂+τ): anula el DESPLAZAMIENTO en t̂=0 (robusto,
    # sin colapso) pero NO la velocidad → la red debe imponer u̇(0)=0 globalmente (vía
    # loss_ic). Esto es CLAVE para romper el sesgo cuasi-estático: la oscilación nace de
    # reconciliar la deflexión de la carga con el reposo inicial; si u̇(0)=0 se cumple "por
    # construcción" localmente, la red nunca necesita oscilar (ver docs/05 §4-quinquies).
    hard_ic: bool = False
...
class EntrenamientoCfg:
    ...
    # CURRÍCULO TEMPORAL (estrategia (2), docs/05 §4-quinquies): el entrenamiento Adam se
    # hace en ETAPAS con ventana [0, t_frac·T_hat] creciente. La red clava primero el
    # transitorio inicial y luego lo extiende → respeta la CAUSALIDAD temporal (como el
    # FEM, que marcha en el tiempo) y deja emerger la oscilación, que el ajuste global
    # "todos los instantes a la vez" no fija. Cada par es (t_frac, fracción_de_épocas_Adam).
    curriculum_stages: tuple = ((0.3, 0.3), (0.6, 0.3), (1.0, 0.4))

    # PESADO CAUSAL del residuo (Wang et al. 2022; ver pde_loss._causal_residual y
    # docs/05 §4-septies). Es el fix de fondo del NO-REBOTE: la oscilación es vibración
    # libre conservativa cuya señal en el residuo (~ω̂²≈0.018) cae bajo el suelo de
    # convergencia → una pérdida global no la "ve" y elige la solución cuasi-estática
    # (que viola la CI de reposo). El pesado causal w_i=exp(-ε·Σ_{j<i}L_j) fuerza a
    # resolver el residuo en ORDEN temporal, propagando el reposo inicial hacia adelante
    # (como un FEM que marcha) y haciendo emerger el rebote. Complementa al currículo
    # (que crece la ventana) ordenando DENTRO de cada ventana.
    causal: bool = True
    eps_causal: float = 1.0       # ε: agresividad del orden temporal (clave; autopaceado)
    n_causal_bins: int = 32       # nº de franjas temporales para acumular el residuo
    ...
    # CI con peso ALTO: con hard_ic=False la velocidad inicial u̇(0)=0 ya NO se cumple por
    # construcción, sino que es esta pérdida la que la impone → motor de la oscilación.
    w_ic: float = 10.0     # condición inicial (ahora ACTIVA: reposo en velocidad)
    # Equilibrio seccional: impone la FORMA del momento M̂=K·(1-ξ)·g(t̂) (máx. en el
    # empotramiento) y del cortante. Es física EXACTA a todo instante (∇·σ=0) y el único
    # término que fija la distribución de tensión; se impone PLENO en toda la ventana.
    # (Histórico docs/05 §4-sexies: una relajación temporal exp(-(t̂-t_ramp)/τ) que lo
    # apagaba tras la rampa REGRESIONÓ la estática —σ_VM máx. al centro, magnitud ~40 %—
    # sin añadir rebote; revertida. El rebote NO viene de soltar el equilibrio.)
    w_eq: float = 5.0      # equilibrio seccional (forma del momento/cortante, pleno en t̂)
