...
class PINNsformer(nn.Module):
    ...
    def __init__(self, d_in, d_out, d_model, d_hidden, N, heads, in_lo=None, in_hi=None,
                 hard_bc=False, ic_tau=1.0):
        super().__init__()

        # Restricciones DURAS de IC/empotramiento (ver config.ModeloCfg).
        # ic_tau = constante de subida de la envolvente temporal SATURANTE.
    ...
    def forward(self, xi, eta, t):
        ...
        # Ansatz de restricción dura: U,V = ξ · t̂²/(t̂²+τ²) · Ñ(...). El factor se
        # anula en ξ=0 (empotramiento) y en t̂=0 (IC); cerca de t̂=0 va como (t̂/τ)²,
        # así que anula desplazamiento Y velocidad. A diferencia del antiguo (t̂/T̂)²,
        # SATURA a 1 para t̂≫τ en vez de crecer con el horizonte (clave en el régimen
        # de flexión, con T_hat grande). La autodiferenciación atraviesa este factor:
        # el residuo y las tensiones se calculan sobre la solución ya restringida;
        # train e inferencia quedan automáticamente consistentes.
        if self.hard_bc:
            t2 = t * t                                         # (N, k, 1)
            envelope = t2 / (t2 + self.ic_tau * self.ic_tau)   # ~ (t̂/τ)² en 0, → 1
            factor = xi * envelope                             # 0 en ξ=0 y t̂=0
