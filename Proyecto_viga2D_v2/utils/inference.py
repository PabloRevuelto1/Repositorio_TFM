...
def load_model(checkpoint_path=None, device="cpu"):
    ...
    model = PINNsformer(
        ...
        in_lo=lo, in_hi=hi,
        # Restricciones duras: el flag y T_hat viven en el checkpoint (compat. con
        # checkpoints antiguos que no los traían → hard_bc desactivado).
        hard_bc=cfg.get("hard_constraints", False),
        t_hat_max=ckpt.get("T_hat", 1.0),
    ).to(device)
...
def evaluate_fields(model, material, L, c, P_max, t_imp_hat,
                    nx=45, ny=15, nt=40, device="cpu"):
    """
    Evalúa los campos en una malla (nt, ny, nx) para una viga concreta.

    Parámetros físicos:
        L      : longitud [m]
        c      : semi-altura [m]
        P_max  : carga cortante máxima [N/m]
        t_imp_hat : instante de impacto adimensional t̂_imp
    ...
    base = np.stack([
        XX.ravel(), EE.ravel(), TT.ravel(),
        np.full(XX.size, L_hat), np.full(XX.size, c_hat),
        np.full(XX.size, p_hat), np.full(XX.size, t_imp_hat),
    ...
    for s in range(0, seq_full.shape[0], chunk):
        ...
        t_t, params = seq[:, :, 2:3], seq[:, :, 3:7]
        with torch.enable_grad():
            out = model(xi_t, eta_t, t_t, params)
            U, V = out[:, :, 0:1], out[:, :, 1:2]
