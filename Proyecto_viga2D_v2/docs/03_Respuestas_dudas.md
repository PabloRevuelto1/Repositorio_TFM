...
## 9. Las 7 entradas de la red: ¿los parámetros son fijos o varían?

Cada muestra que entra a la red es el vector
...
### 11.2 ¿Por qué la simulación dura siempre 0.77 ms, ponga lo que ponga en $\hat t_{\mathrm{imp}}$?

Primero, una precisión: el eje de la animación está en **milisegundos**, así que
son **0.77 ms**, no 0.77 s. Y es **constante** por construcción.

El horizonte temporal de la simulación es **fijo**: la malla siempre va de
$\hat t = 0$ a $\hat t = \hat T = 4.0$ ([config.py](../config.py#L120),
[inference.py](../utils/inference.py#L72)). Al pasar a tiempo físico:

$$t_{\mathrm{fis}} = \hat t \cdot T_{\mathrm{ref}}, \qquad
  T_{\mathrm{ref}} = L_{\mathrm{ref}}\sqrt{\frac{\rho}{E}}$$

Para el **acero**: $$T_{\mathrm{ref}} = 1\cdot\sqrt{\frac{7850}{210\cdot10^9}} \approx 1.93\cdot10^{-4},\quad
\text{luego la ventana dura:}\quad
\hat T\cdot T_{\mathrm{ref}} = 4\cdot1.93\cdot10^{-4} \approx 0.77$$ms. Siempre.

¿Y $\hat t_{\mathrm{imp}}$? **No cambia la duración de la ventana**, solo cambia
**cuándo** golpea el impacto *dentro* de ella. La carga es
$\propto\mathrm{sigmoid}(\alpha(\hat t-\hat t_{\mathrm{imp}}))$: subir
$\hat t_{\mathrm{imp}}$ retrasa el golpe (la viga arranca quieta más tiempo), pero el
vídeo sigue cubriendo $[0,\,4]$ adimensional. Como $\hat t_{\mathrm{imp}}\in[0.5,1.5]$
cae bien dentro de $[0,4]$, el impacto y la oscilación posterior siempre se ven.

> Lo único que **sí** cambia la duración es el **material** (vía $T_{\mathrm{ref}}$):
> con aluminio, $T_{\mathrm{ref}}\approx1.96\cdot10^{-4}$s → ventana $\approx0.79$ ms.
...
- Si la **flecha PINN es ~1000× menor** que la de Euler-Bernoulli (p. ej. 0.16 µm
  frente a 800 µm) y la **frecuencia está totalmente desfasada**, no es que
  Euler-Bernoulli "falle": es que la **red colapsó a la solución casi trivial**
  $u,v\approx0$ y apenas se deforma. Eso es exactamente lo que detectaron estas
  métricas en el primer entrenamiento, y la razón del **plan de arreglo**
  (restricciones duras de empotramiento/inicial + más peso a la carga + reescalado
  del residuo) descrito en el documento de defensa. Aquí Euler-Bernoulli cumple su
  papel: es la **vara de medir** que delata que el modelo no se había deformado.
...
> PINN se mueve en el rango físico correcto o se ha quedado "plana".
...
11. Visualización: deformada en $t=0$ = **IC blanda + exageración** (no bug); 0.77 **ms** fijo porque $\hat T=4$ no depende de $\hat t_{\mathrm{imp}}$; color = Von Mises (dónde sufre); barritas = campo $(u,v)$ en movimiento.
