...
│   ├── metrics.py         # Métricas de simulación + validación física (§12/§13)
...
- **Métricas de entrenamiento** (panel lateral): semáforo de calidad del modelo,
  desglose de pérdidas finales, curva de convergencia (log) y datos del modelo.
- **Mapa animado:** malla deformada coloreada por **tensión de Von Mises**, con
  Play/Pause y slider temporal nativos de Plotly. El estado indica el **factor de
  exageración** aplicado a la deformada (los desplazamientos reales son micras).
- **Gráfica 1:** desplazamiento del extremo libre $v(L,0,t)$, con marca del
  **instante de impacto** $\hat{t}_{\mathrm{imp}}$.
- **Gráfica 2:** $\sigma_{xx}(x)$ a lo largo de la viga en el instante seleccionado.
- **Métricas de simulación:** flecha máxima/estática, frecuencia y periodo de
  oscilación, $\sigma_{xx}$ y $\sigma_{VM}$ máximas (con su posición), deformación
  máxima y **factor de seguridad** $\sigma_y/\sigma_{VM}$ con veredicto.
- **Validación física** (docs/03 §12): *sanity-checks* (reposo en $t=0$,
  empotramiento fijo, superficies libres), contraste con **Euler-Bernoulli**
  (flecha y frecuencia) y **residuo de la EDP** en malla nueva (generalización).
...
- `referencia_analitica` (bool): activa/desactiva el contraste Euler-Bernoulli
  (Nivel 3). Es una fórmula cerrada, de coste despreciable.
- `residuo_en_malla` (bool) y `n_residuo_check`: residuo de la EDP en malla nueva
  (Nivel 4); usa autograd de 2.º orden sobre un subconjunto pequeño de puntos.

### ¿Cómo sé si el modelo es bueno? (sin *ground truth*)

En una PINN **la verdad son las ecuaciones**, no datos etiquetados. El modelo es
fiable si: las **pérdidas finales** son bajas (semáforo verde), los
**sanity-checks** pasan (viga plana en $t=0$, empotramiento quieto), la **flecha
y frecuencia** concuerdan con Euler-Bernoulli y el **residuo en malla nueva** es
pequeño (generaliza). Todo ello se lee de un vistazo en la app
([docs/03 §12](docs/03_Respuestas_dudas.md)).
...
- **Pesos de pérdida** ($w_{bc}$, $w_{ic}$) para equilibrar contorno/inicial frente
  al residuo.
- **Validación física sin *ground truth*** (`utils/metrics.py`): métricas de
  simulación, *sanity-checks*, contraste con Euler-Bernoulli y residuo en malla
  nueva — para juzgar el modelo con criterio, no "a ojo".
