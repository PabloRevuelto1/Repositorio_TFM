...
def main():
    ...
    model = PINNsformer(
        ...
        in_lo=lo, in_hi=hi,
        hard_bc=mod.hard_constraints, t_hat_max=dom.T_hat,
    ).to(device)
    ...
    def compute_grads():
        ...
        for p in model.parameters():
            p.grad = None
        comps = {"res": 0.0, "load": 0.0, "bc": 0.0, "ic": 0.0}
    ...
    try:
        ...
        if args.lbfgs > 0 and not stop["flag"]:
            ...
            t0 = time.time()
            hold = {"comps": {"res": 0.0, "load": 0.0, "bc": 0.0, "ic": 0.0}}
    ...
    payload = {
        ...
        "loss_weights": {"res": ent.w_res, "load": ent.w_load, "bc": ent.w_bc, "ic": ent.w_ic},
        "history": history_sub,
